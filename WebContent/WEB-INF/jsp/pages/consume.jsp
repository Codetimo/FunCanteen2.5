...
<form class="api-form" method="post" action="${pageContext.request.contextPath}/runout.action" target="_blank">
<p>
<label>商户号：</label>
<input id="merId" type="text" name="merId" placeholder="" value="777290058155173" title="商户号码（商户号码777290058110097仅做为测试调通交易使用，该商户号配置了需要对敏感信息加密）测试时请改成自己申请的商户号，【自己注册的测试777开头的商户号不支持代收产品】" required="required"/>
</p>
<p>
<label>交易金额：</label>
<input id="txnAmt" type="text" name="txnAmt" placeholder="交易金额" value="${price}" title="单位为分,不能带小数点 " required="required"/>
</p>
<p>
<label>订单发送时间：</label>
...
<hr />
 <jsp:include  page="/WEB-INF/jsp/pages/more_faq.jsp"/>
</div>
